/*
 * pomerance_x16_sylow_descent_barrett_full.c — X1(16) Nonsplit Filter + Successive Halving
 *                                                Descent, FULL Barrett-Reduction Rewrite
 *
 * ARCHITECTURE:
 ...
 *      Outputs both the Montgomery/Barrett coefficient A and the rational 16-torsion
 *      point xP16.
 ...
 *   around 10^5-10^6 per triple — is the *sampling and filtering* loop:
 *     - x16_y_predicts_nonsplit (Legendre-symbol modular exponentiation), run on every
 *       sampled y.
 *     - sqrtmod_p5 (another modular exponentiation), run during sampling AND at every
 *       halving step.
 *     - the halving steps themselves (several modular multiplications per branch, at
 *       every depth of the climb).
 *   All of these ran through 128-bit Montgomery arithmetic even when p easily fits in
 *   64 bits. This file adds a *complete parallel u64 pipeline* — sampling, nonsplit
 *   filter, square roots, halving, and verification — built entirely on Barrett-reduced
 *   u64 arithmetic (mulmod64_barrett / powmod64_barrett), selected automatically whenever
 *   p < 2^63. The original u128 Montgomery pipeline is kept unchanged as the fallback for
 *   primes that don't fit in 64 bits. Since the Barrett engine now covers the actual
 *   bottleneck (every sampled curve, not just the rare accepted one), this is expected to
 *   move the needle on wall-clock time, unlike the earlier verify-only patch.
 *
 ...
 *
 * Compile:
 *   gcc -O3 -fopenmp -o pomerance_x16_sylow_descent_barrett_full pomerance_x16_sylow_descent_barrett_full.c -lm -lpthread
 */
...
/* ================================================================
 * Modular Square Root & X1(16) Parametrization — 128-bit (fallback) path
 * ================================================================ */
...
static int x16_sample_curve_nonsplit128(u128 *Ao, u128 *xPo, u128 *pending_A, u128 *pending_xP,
...
/* ================================================================
 * Modular Square Root & X1(16) Parametrization — 64-bit Barrett path
 *
...
static int x16_sample_curve_nonsplit64(u64 *Ao, u64 *xPo, u64 *pending_A, u64 *pending_xP,
                                       int *have_pending_A, Rng *rng, u64 rand_mask,
                                       u64 sqrtm1, const BarrettCtx64 *ctx) {
    u64 p = ctx->p;
    if (*have_pending_A) {
        *Ao = *pending_A;
        *xPo = *pending_xP;
    ...
    for (;;) {
        ...
        if (!x16_y_predicts_nonsplit64(y, y2, ctx)) {
            t_ops.branch_count++;
            continue;
        }
        ...
        u64 sd;
        if (!sqrtmod_p5_64(&sd, D, ctx, sqrtm1)) {
            t_ops.branch_count++;
            continue;
        }
        ...
        for (int ri = 0; ri < 2; ri++) {
            u64 A, xP;
            if (!x16_root_to_montgomery_A64(&A, &xP, roots[ri], y, ctx)) {
                t_ops.branch_count++;
                continue;
            }
            if (!got_A) { first_A = A; first_xP = xP; got_A = 1; }
...
/* ================================================================
 * Successive Halving Descent Engine — 128-bit (fallback) path
 * ================================================================ */
...
/* ================================================================
 * Successive Halving Descent Engine — 64-bit Barrett path
 * ================================================================ */

static int halve_once_first64(u64 *xo, u64 A, u64 x, u64 sqrtm1, const BarrettCtx64 *ctx) {
    u64 p = ctx->p;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        u64 sw;
        if (!sqrtmod_p5_64(&sw, w, ctx, sqrtm1)) {
            t_ops.branch_count++;
            continue;
        }
        u64 candidates[2] = {
...
static int halve_chain_from_depth64(u64 *xout, u64 A, u64 x, int depth, int k, u64 sqrtm1,
                                    const BarrettCtx64 *ctx) {
    for (; depth < k; depth++) {
        if (!halve_once_first64(&x, A, x, sqrtm1, ctx)) return 0;
    }
    if (!verify64_barrett(ctx->p, A, x, ctx)) {
        t_ops.branch_count++;
        return 0;
    }
    *xout = x;
...
#pragma omp parallel
    {
        ...
        while (found_count < target_total && A_trials < budget) {
            u128 A, xP16;
            x16_sample_curve_nonsplit128(&A, &xP16, &pending_A, &pending_xP, &have_pending_A,
...
#pragma omp parallel
    {
        ...
        while (found_count < target_total && A_trials < budget) {
            u64 A, xP16;
            /* STEP 1: Generate X1(16) Curve A with Nonsplit-Discriminant Filter — full
             * Barrett u64 arithmetic, so every sampled curve (not just accepted ones)
             * benefits, unlike the earlier verify-only patch. */
            x16_sample_curve_nonsplit64(&A, &xP16, &pending_A, &pending_xP, &have_pending_A,
                                        &rng, rand_mask, sqrtm1, &ctx);

            A_trials++;
            g_thread_counters[tid].count = A_trials;

            /* STEP 2: Certify via Successive Halving Descent, also fully Barrett-reduced. */
            t_ops.mul_count = 0; t_ops.inv_count = 0; t_ops.branch_count = 0;
            uint64_t c_start = rdtsc();

            u64 xR;
            int verify_ok = halve_chain_from_depth64(&xR, A, xP16, 4, k, sqrtm1, &ctx);
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        printf("Usage: ./pomerance_x16_sylow_descent_barrett_full <stateful_input.txt> <output_pure.csv> <output_metrics.csv> <target_total>\n");
        return 1;
    }
