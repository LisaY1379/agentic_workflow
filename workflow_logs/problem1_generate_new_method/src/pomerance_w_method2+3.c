/*
 * pomerance_x16_halving.c — X1(16) Prescribed Torsion + Successive Halving & Nonsplit Filter
 *
 * PIPELINE: Method 2 + Method 3
 ...
 *      (x16_y_predicts_nonsplit128) to immediately discard split curves where halving must fail.
 ...
 *      roots to climb upward to the top of the 2-Sylow subgroup (depth k).
 *
 ...
 *
 * Compile:
 *   gcc -O3 -fopenmp -o pomerance_x16_halving pomerance_x16_halving.c -lm -lpthread
 */
...
/* FIXED: Reintroduced standard-to-standard modular multiplication helper */
...
/* ================================================================
 * Modular Square Root & Nonsplit X1(16) Sampler
 * ================================================================ */
...
static int x16_sample_nonsplit_curve(u128 *Ao, u128 *xPo, u128 *pending_A, u128 *pending_xP, int *have_pending_A,
...
/* ================================================================
 * Method 3 Halving: Successive Square Root Backtracking Tree
 * ================================================================ */
...
/* Recursive DFS Backtracking: Climbs the 2-Sylow tree from depth 4 to depth k */
...
/* ================================================================
 * Parallel Search Engine: Method 2 (X1(16)) + Method 3 (Halving)
 * ================================================================ */
...
#pragma omp parallel
    {
        ...
        while (found_count < target_total && A_trials < budget) {
            u128 A, xP16;
            /* STEP 1: Sample Nonsplit X1(16) Curve & Marked 16-Torsion Point (Method 2 + Method 3 Filter) */
            x16_sample_nonsplit_curve(&A, &xP16, &pending_A, &pending_xP, &have_pending_A, &rng, p, rand_mask, sqrtm1, &mt);

            /* INCREMENT ONCE PER VALID NONSPLIT CURVE SAMPLED */
            A_trials++;
            ...
            /* STEP 2: Successive Halving Backtracking (Method 3 Halving)
             * Starts from marked point xP16 (depth 4) and climbs up to depth k.
             * Tree branching and intermediate square roots DO NOT increment A_trials. */
            u128 xR;
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        printf("Usage: ./pomerance_x16_halving <stateful_input.txt> <output_pure.csv> <output_metrics.csv> <target_total>\n");
        return 1;
    }
