/*
 * pomerance_projection_benchmarked.c — Incremental generation of Pomerance triples (p, A, x0)
 *
 * This keeps the full 2-Sylow projection (Ruehle) search:
 *   - candidate group orders N = p+1 ± t = 2^k * m are precomputed (compute_odd_parts)
 *   - a random point is multiplied by the odd part m (xMUL)
 *   - ~k doublings decide whether the result lands in the 2-Sylow subgroup
 *   - surviving candidates are handed to verify64/verify128 for final confirmation
 *
 * Added on top of the original algorithm: three benchmarks captured per
 * successful candidate / prime batch:
 *
 *   1. CPU cycles (rdtsc)   — cycles spent in the full per-candidate pipeline
 *                             (odd-part multiply -> doubling test -> extract
 *                             xR -> final verify) for the candidate that
 *                             actually succeeded.
 *   2. OpCount (M3)         — weighted "algorithmic work" for that same
 *                             pipeline: modular multiplications (mm64/mm128/
 *                             mulmod64/mulmod_slow), modular inversions
 *                             (the Fermat's-little-theorem inverse used to
 *                             extract xR, and inside verify), and branch
 *                             decisions (early rejections at each stage).
...
/* ================================================================
 * Benchmark #1: CPU cycle counting via rdtsc
 * ================================================================ */
...
/* Thread-local: reset immediately before each candidate's pipeline runs,
 * snapshotted immediately after. No locking needed. */
...
/* Modular inverse via Fermat's little theorem — counted as ONE inversion
 * (inv_count) even though internally it costs ~log2(p) multiplications
 * (which are separately counted via mulmod64 -> mul_count). */
...
/* Modular inverse in Montgomery form, again counted as ONE inversion. */
...
/* ================================================================
 * Shared: compute_k, odd parts (unchanged — per-prime setup, not
 * part of any individual candidate's benchmark)
 * ================================================================ */
...
/* ================================================================
 * Dispatch: search64 / search128 (2-SYLOW PROJECTION, BENCHMARKED)
 * ================================================================ */
...
int main(int argc, char *argv[]) {
    ...
    /* Added batch_time_ms to CSV header for downstream Python analysis */
